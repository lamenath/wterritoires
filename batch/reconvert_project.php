<?php
	...
	**/
	...
	foreach($contacts as $contact)
	{
		...
		if(file_exists(sfConfig::get('sf_upload_dir').'/projet/max_'.$fileName))
			continue;
		...
		try {
			...
			$thumbnail->loadFile(sfConfig::get('sf_upload_dir').'/projet/' . $fileName);
			$thumbnail->save(sfConfig::get('sf_upload_dir').'/projet/max_'.$fileName, 'image/png');

			// Create the thumbnail
			$thumbnail = new sfThumbnail(95, 60,  false, false, 80, "sfImageMagickAdapter", array('convert' => '/usr/local/bin/convert', 'method' => 'shave_all'));
			$thumbnail->loadFile(sfConfig::get('sf_upload_dir').'/projet/' . $fileName);
			$thumbnail->save(sfConfig::get('sf_upload_dir').'/projet/normal_'.$fileName, 'image/png');

			// Create the thumbnail
			$thumbnail = new sfThumbnail(60, 60, false, false, 80, "sfImageMagickAdapter", array('convert' => '/usr/local/bin/convert', 'method' => 'shave_all'));
			$thumbnail->loadFile(sfConfig::get('sf_upload_dir').'/projet/' . $fileName);
			$thumbnail->save(sfConfig::get('sf_upload_dir').'/projet/mini_'.$fileName, 'image/png');
		}
		catch(Exception $e)
		{
