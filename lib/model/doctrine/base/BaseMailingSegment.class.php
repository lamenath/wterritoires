<?php

/**
 * BaseMailingSegment
 * 
 * This class has been auto-generated by the Doctrine ORM Framework
 * 
 * @property integer $id
 * @property integer $segment_id
 * @property integer $mailing_id
 * @property Segment $Segment
 * @property Mailing $Mailing
 * 
 * @method integer        getId()         Returns the current record's "id" value
 * @method integer        getSegmentId()  Returns the current record's "segment_id" value
 * @method integer        getMailingId()  Returns the current record's "mailing_id" value
 * @method Segment        getSegment()    Returns the current record's "Segment" value
 * @method Mailing        getMailing()    Returns the current record's "Mailing" value
 * @method MailingSegment setId()         Sets the current record's "id" value
 * @method MailingSegment setSegmentId()  Sets the current record's "segment_id" value
 * @method MailingSegment setMailingId()  Sets the current record's "mailing_id" value
 * @method MailingSegment setSegment()    Sets the current record's "Segment" value
 * @method MailingSegment setMailing()    Sets the current record's "Mailing" value
 * 
 * @package    rrr
 * @subpackage model
 * @author     Your name here
 * @version    SVN: $Id: Builder.php 7490 2010-03-29 19:53:27Z jwage $
 */
abstract class BaseMailingSegment extends sfDoctrineRecord
{
    public function setTableDefinition()
    {
        $this->setTableName('mailing_segment');
        $this->hasColumn('id', 'integer', null, array(
             'type' => 'integer',
             'primary' => true,
             'autoincrement' => true,
             ));
        $this->hasColumn('segment_id', 'integer', null, array(
             'type' => 'integer',
             'primary' => true,
             ));
        $this->hasColumn('mailing_id', 'integer', null, array(
             'type' => 'integer',
             'primary' => true,
             ));

        $this->option('collate', 'utf8_unicode_ci');
        $this->option('charset', 'utf8');
    }

    public function setUp()
    {
        parent::setUp();
        $this->hasOne('Segment', array(
             'local' => 'segment_id',
             'foreign' => 'id',
             'onDelete' => 'CASCADE'));

        $this->hasOne('Mailing', array(
             'local' => 'mailing_id',
             'foreign' => 'id',
             'onDelete' => 'CASCADE'));

        $timestampable0 = new Doctrine_Template_Timestampable();
        $this->actAs($timestampable0);
    }
}