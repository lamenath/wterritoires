<script type="text/javascript">
	...
	function addMarkers()
	{
		...
		function createMarker(typ, id, title, url, bio, city, pic, lat, lon)
		{
			var marker = new google.maps.Marker({
				map: map,
				icon: (typ == "user" ? BuddyImage : ProjectImage),
				draggable: false,
				title: title,
		...
		$.ajax({
			url: "<?php echo url_for("json/feed"); ?>",
			dataType: "json",
			type: "GET",
	...
	function initialize()
	{
		...
		default_pos = new google.maps.LatLng(<?php echo $sf_user->getObject()->getLatitude(); ?>, <?php echo $sf_user->getObject()->getLongitude(); ?>);
